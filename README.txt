												Code Overview
*** I edited the csv to be able to parse through easily, it can be found in bin/debug/books.csv		The xml file can also be found in that same folder ***
...
Book.cs:	Book Declaration:          Standard book class declaration with generic parameters from csv file, last parameter is an int which is 0 if in library database or 1 if requested. 
					   I wanted to use a book but that's not a bindable data type apparently.
